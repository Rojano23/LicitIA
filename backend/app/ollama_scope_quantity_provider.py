...
OLLAMA_SCOPE_QUANTITY_PROMPT_VERSION = "scope-quantity-semantic-discovery-2026-09-08-001"
...
class OllamaScopeQuantitySemanticDiscoveryProvider(ScopeQuantitySemanticDiscoveryProvider):
    ...
    def _build_prompt(fragment: ScopeQuantitySemanticFragment) -> str:
        return (
            ...
            "Use quantity_raw as literal source text token, do not normalize literal evidence words. "
            "Use unit_raw as literal grounded unit text when available. "
            ...
            "Normalized numeric fields must be strings using safe decimal syntax, e.g. 1, 2, 2.5, 100. Never use commas or ranges in a single numeric field. "
            "For RANGE, use quantity_min_raw and quantity_max_raw, do not invent missing bounds. "
            "For mixed context, keep only execution quantities. Example: 'Se suministrarán 2 válvulas de 4 pulgadas, clase 300.' returns one COUNT quantity for 2 válvulas; do not emit 4 pulgadas or 300. Example: 'Instalar 500 m de cable de 5 mm.' returns LENGTH 500 m only. "
