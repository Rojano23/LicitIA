...
@app.post("/tenders/{tender_id}/analyze-changes", response_model=TenderChangesRead)
def analyze_tender_changes_endpoint(
    tender_id: str,
    db: Session = Depends(get_db),
) -> dict[str, object]:
    tender = db.get(Tender, tender_id)
    ...
    try:
        payload = analyze_tender_changes(db, tender_id)
        db.commit()
        return payload
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
@app.get("/tenders/{tender_id}/changes", response_model=TenderChangesRead)
def get_tender_changes_endpoint(
    tender_id: str,
    db: Session = Depends(get_db),
) -> dict[str, object]:
    tender = db.get(Tender, tender_id)
...
@app.patch("/tenders/{tender_id}/changes/{change_id}", response_model=TenderChangesRead)
def update_tender_change_endpoint(
    tender_id: str,
    change_id: str,
    payload: TenderChangeDecisionWrite,
    db: Session = Depends(get_db),
) -> dict[str, object]:
    tender = db.get(Tender, tender_id)
    if tender is None:
    ...
    try:
        result = update_tender_change_human_decision(
            db,
            tender_id=tender_id,
            change_id=change_id,
            action=payload.action,
...
@app.get("/tenders/{tender_id}/effective-state", response_model=TenderEffectiveStateRead)
def get_tender_effective_state_endpoint(
    tender_id: str,
    db: Session = Depends(get_db),
) -> dict[str, object]:
    tender = db.get(Tender, tender_id)
...
@app.post("/tenders/{tender_id}/analyze-requirements", response_model=TenderRequirementCandidatesRead)
def analyze_tender_requirements_endpoint(
    ...
    try:
        payload = analyze_tender_requirements(db, tender_id)
        db.commit()
        return payload
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
@app.get("/tenders/{tender_id}/requirement-candidates", response_model=TenderRequirementCandidatesRead)
def get_tender_requirement_candidates_endpoint(
    tender_id: str,
    db: Session = Depends(get_db),
) -> dict[str, object]:
    tender = db.get(Tender, tender_id)
