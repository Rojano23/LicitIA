...
from app.scope_attribute_adapters import (
    ...
    default_scope_attribute_adapters,
    run_scope_attribute_adapter,
)
from app.scope_attributes import (
    replace_scope_attributes_for_artifact,
    validate_scope_attribute_candidate,
)

SCOPE_ATTRIBUTE_MATERIALIZATION_STATUS_MATERIALIZED = "MATERIALIZED"
SCOPE_ATTRIBUTE_MATERIALIZATION_STATUS_NO_ATTRIBUTES = "NO_ATTRIBUTES"
SCOPE_ATTRIBUTE_MATERIALIZATION_STATUS_UNSUPPORTED = "UNSUPPORTED"
SCOPE_ATTRIBUTE_MATERIALIZATION_STATUS_REVIEW_REQUIRED = "REVIEW_REQUIRED"
SCOPE_ATTRIBUTE_MATERIALIZATION_STATUS_INVALID_EVIDENCE = "INVALID_EVIDENCE"


@dataclass(frozen=True, slots=True)
class ScopeAttributeMaterializationResult:
    scope_detail_id: str
...
def materialize_scope_attributes_for_scope_detail(
    db: Session,
    *,
    scope_detail_id: str,
    adapters: Sequence[ScopeAttributeAdapter] | None = None,
) -> ScopeAttributeMaterializationResult:
    scope_detail = db.get(TenderScopeDetail, scope_detail_id)
    if scope_detail is None:
        return ScopeAttributeMaterializationResult(
            scope_detail_id=scope_detail_id,
            source_artifact_key=None,
            adapter_name=None,
            adapter_version=None,
            status=SCOPE_ATTRIBUTE_MATERIALIZATION_STATUS_INVALID_EVIDENCE,
            candidate_count=0,
            persisted_count=0,
    ...
    try:
        artifact = _resolve_scope_attribute_evidence_artifact(db, scope_detail=scope_detail)
    ...
    extraction = run_scope_attribute_adapter(
        db,
        artifact,
        adapters=tuple(adapters or default_scope_attribute_adapters()),
    )

    if extraction.status == SCOPE_ATTRIBUTE_ADAPTER_STATUS_UNSUPPORTED:
        return ScopeAttributeMaterializationResult(
            scope_detail_id=scope_detail.id,
            source_artifact_key=source_artifact_key,
            adapter_name=extraction.adapter_name,
            adapter_version=extraction.adapter_version,
            status=SCOPE_ATTRIBUTE_MATERIALIZATION_STATUS_UNSUPPORTED,
            candidate_count=0,
            persisted_count=0,
            review_required_count=0,
    ...
    if extraction.status == SCOPE_ATTRIBUTE_ADAPTER_STATUS_INVALID_EVIDENCE:
        return ScopeAttributeMaterializationResult(
            scope_detail_id=scope_detail.id,
            source_artifact_key=source_artifact_key,
            adapter_name=extraction.adapter_name,
    ...
    try:
        persisted = replace_scope_attributes_for_artifact(
            db,
    ...
    if extraction.status == SCOPE_ATTRIBUTE_ADAPTER_STATUS_NO_ATTRIBUTES:
        status = SCOPE_ATTRIBUTE_MATERIALIZATION_STATUS_NO_ATTRIBUTES
    elif review_required_count > 0 or extraction.status == SCOPE_ATTRIBUTE_ADAPTER_STATUS_REVIEW_REQUIRED:
        status = SCOPE_ATTRIBUTE_MATERIALIZATION_STATUS_REVIEW_REQUIRED
    else:
        status = SCOPE_ATTRIBUTE_MATERIALIZATION_STATUS_MATERIALIZED
...
def _resolve_scope_attribute_evidence_artifact(
    db: Session,
    *,
    ...
    if source_method == "VISION":
        ...
        page = db.get(DocumentPage, ocr_row.document_page_id)
        if page is None or page.document_id != scope_detail.source_document_id:
            raise ValueError("source artifact OCR result is outside scope detail document")
        ...
        if page is None:
            raise ValueError("source artifact page does not exist")
        if page.id != scope_detail.document_page_id:
            raise ValueError("source artifact page is outside scope detail page")
        if page.document_id != scope_detail.source_document_id:
            raise ValueError("source artifact page is outside scope detail document")
