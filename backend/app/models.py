...
class RequirementComplianceSystemStatus(str, Enum):
    SUPPORTED = "SUPPORTED"
...
class RequirementComplianceApplicabilityContext(str, Enum):
    APPLIES = "APPLIES"
    CONDITION_UNRESOLVED = "CONDITION_UNRESOLVED"
    UNKNOWN = "UNKNOWN"


class RequirementComplianceCheckStatus(str, Enum):
    PASS = "PASS"
    FAIL = "FAIL"
    UNKNOWN = "UNKNOWN"
...
class Company(Base):
    ...
    compliance_assessments: Mapped[list["RequirementComplianceAssessment"]] = relationship(
        back_populates="company",
        cascade="all, delete-orphan",
    )
...
class RequirementComplianceAssessment(Base):
    __tablename__ = "requirement_compliance_assessments"
    ...
    __table_args__ = (
        ...
        Index("ix_req_compliance_assessment_tender_id", "tender_id"),
        Index("ix_req_compliance_assessment_company_id", "company_id"),
        Index("ix_req_compliance_assessment_requirement_id", "requirement_id"),
        Index("ix_req_compliance_assessment_status", "system_status"),
        Index("ix_req_compliance_assessment_applicability", "applicability_context"),
    )
    ...
    applicability_context: Mapped[str] = mapped_column(String(32), nullable=False)
    assessment_summary: Mapped[str] = mapped_column(Text, nullable=False, default="")
    ...
    tender: Mapped["Tender"] = relationship(back_populates="compliance_assessments")
    company: Mapped[Company] = relationship(back_populates="compliance_assessments")
    requirement: Mapped["Requirement"] = relationship(back_populates="compliance_assessments")
    checks: Mapped[list["RequirementComplianceCheck"]] = relationship(
        back_populates="assessment",
        cascade="all, delete-orphan",
    )


class RequirementComplianceCheck(Base):
    __tablename__ = "requirement_compliance_checks"

    __table_args__ = (
        Index("ix_req_compliance_check_assessment_id", "assessment_id"),
        Index("ix_req_compliance_check_type", "check_type"),
        Index("ix_req_compliance_check_status", "check_status"),
        Index("ix_req_compliance_check_company_evidence_id", "company_evidence_id"),
        Index("ix_req_compliance_check_match_id", "match_id"),
    )

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid4()))
    assessment_id: Mapped[str] = mapped_column(
        String(36),
        ForeignKey("requirement_compliance_assessments.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    tender_id: Mapped[str] = mapped_column(
    ...
    company_evidence_id: Mapped[str | None] = mapped_column(
        String(36),
        ForeignKey("company_evidence.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    match_id: Mapped[str | None] = mapped_column(
        String(36),
        ForeignKey("requirement_evidence_candidate_matches.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    created_at: Mapped[datetime] = mapped_column(
    ...
    assessment: Mapped[RequirementComplianceAssessment] = relationship(back_populates="checks")
    tender: Mapped["Tender"] = relationship(back_populates="compliance_checks")
    company: Mapped[Company] = relationship(back_populates="compliance_checks")
    requirement: Mapped["Requirement"] = relationship(back_populates="compliance_checks")
    company_evidence: Mapped[CompanyEvidence | None] = relationship()
...
class Tender(Base):
    ...
    tender_scope_attributes: Mapped[list["TenderScopeAttribute"]] = relationship(
        back_populates="tender",
        cascade="all, delete-orphan",
    )
    tender_scope_quantities: Mapped[list["TenderScopeQuantity"]] = relationship(
        back_populates="tender",
        cascade="all, delete-orphan",
    )
...
class TenderScopeDetail(Base):
    ...
    scope_attributes: Mapped[list["TenderScopeAttribute"]] = relationship(
        back_populates="scope_detail",
        cascade="all, delete-orphan",
    )
    scope_quantities: Mapped[list["TenderScopeQuantity"]] = relationship(
        back_populates="scope_detail",
        cascade="all, delete-orphan",
    )


class TenderScopeAttribute(Base):
    __tablename__ = "tender_scope_attributes"
    ...
    __table_args__ = (
        ...
        Index("ix_tender_scope_attributes_tender_id", "tender_id"),
        Index("ix_tender_scope_attributes_scope_detail_id", "scope_detail_id"),
        Index("ix_tender_scope_attributes_source_document_id", "source_document_id"),
        Index("ix_tender_scope_attributes_document_page_id", "document_page_id"),
        Index("ix_tender_scope_attributes_source_artifact_key", "source_artifact_key"),
        Index("ix_tender_scope_attributes_attribute_name", "attribute_name"),
    )
    ...
    scope_detail_id: Mapped[str] = mapped_column(
        String(36),
        ForeignKey("tender_scope_details.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    source_document_id: Mapped[str] = mapped_column(
    ...
    attribute_label_raw: Mapped[str | None] = mapped_column(String(255), nullable=True)
    normalized_name: Mapped[str | None] = mapped_column(String(128), nullable=True)
    value_raw: Mapped[str] = mapped_column(String(255), nullable=False)
    unit_raw: Mapped[str | None] = mapped_column(String(128), nullable=True)
    ...
    tender: Mapped[Tender] = relationship(back_populates="tender_scope_attributes")
    scope_detail: Mapped[TenderScopeDetail] = relationship(back_populates="scope_attributes")
    source_document: Mapped[TenderDocument] = relationship()
    document_page: Mapped[DocumentPage] = relationship()
    source_analysis: Mapped[DocumentVisionAnalysis | None] = relationship()
    source_page_result: Mapped[DocumentVisionPageResult | None] = relationship()
...
class TenderScopeQuantity(Base):
    __tablename__ = "tender_scope_quantities"
    ...
    __table_args__ = (
        ...
        Index("ix_tender_scope_quantities_tender_id", "tender_id"),
        Index("ix_tender_scope_quantities_scope_detail_id", "scope_detail_id"),
        Index("ix_tender_scope_quantities_source_document_id", "source_document_id"),
        Index("ix_tender_scope_quantities_document_page_id", "document_page_id"),
        Index("ix_tender_scope_quantities_source_artifact_key", "source_artifact_key"),
        Index("ix_tender_scope_quantities_measure_kind", "measure_kind"),
        Index("ix_tender_scope_quantities_relation", "relation"),
    )
    ...
    quantity_raw: Mapped[str] = mapped_column(String(128), nullable=False)
    quantity_value: Mapped[Decimal | None] = mapped_column(Numeric(18, 4), nullable=True)
    quantity_min: Mapped[Decimal | None] = mapped_column(Numeric(18, 4), nullable=True)
    quantity_max: Mapped[Decimal | None] = mapped_column(Numeric(18, 4), nullable=True)
    unit_raw: Mapped[str | None] = mapped_column(String(128), nullable=True)
    measure_kind: Mapped[str | None] = mapped_column(String(32), nullable=True)
    relation: Mapped[str | None] = mapped_column(String(32), nullable=True)
    source_method: Mapped[str] = mapped_column(String(32), nullable=False)
    source_artifact_key: Mapped[str] = mapped_column(String(255), nullable=False)
    source_locator: Mapped[str] = mapped_column(String(512), nullable=False)
    ...
    tender: Mapped[Tender] = relationship(back_populates="tender_scope_quantities")
    scope_detail: Mapped[TenderScopeDetail] = relationship(back_populates="scope_quantities")
    source_document: Mapped[TenderDocument] = relationship()
    document_page: Mapped[DocumentPage] = relationship()
    source_analysis: Mapped[DocumentVisionAnalysis | None] = relationship()
    source_page_result: Mapped[DocumentVisionPageResult | None] = relationship()
